...
do
  for seed in 1 2 3 
  do
  	python src/DeepCore/main.py --data_path $data_path --dataset $dataset --n-class $n_class --model $model --selection $selection --epochs $epochs --batch-size $batch_size --balance $balance --fraction $fraction --seed $seed --per_epoch $per_epoch --lr $lr --scheduler $scheduler --test_interval $test_interval >& distillation_${dataset}_${model}_${selection}_${balance}_${fraction}_${seed}.out
  done
done
