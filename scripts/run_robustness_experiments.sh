...
do
        ...
        do
                ...
                do
                        python src/DeepCore/main.py --data_path $data_path --dataset $dataset --n-class $n_class --model $model --selection $selection --epochs $epochs --batch-size $batch_size --balance $balance --fraction $fraction --seed $seed --per_epoch $per_epoch --resolution $resolution --test_interval $test_interval --save_path $chpt_path  --noisy_dataset_path $noisy_dataset_path >& $out_path/timeToAcc_${per_epoch}_${resolution}_${test_interval}_${dataset}_${model}_${selection}_balance${balance}_hard${use_hard_examples}_${fraction}_${seed}.out
                done
        done
done
...
do
        ...
        do
                for noisy_dataset_path in "noisycifar10_10.pt" "noisycifar10_30.pt" "noisycifar10_50.pt"
                do
                        python src/DeepCore/main.py --data_path $data_path --dataset $dataset --n-class $n_class --model $model --selection $selection --uncertainty $uncertainty --epochs $epochs --batch-size $batch_size --balance $balance --fraction $fraction --seed $seed --per_epoch $per_epoch --resolution $resolution --test_interval $test_interval --save_path $chpt_path  --noisy_dataset_path $noisy_dataset_path >& $out_path/timeToAcc_${per_epoch}_${resolution}_${test_interval}_${dataset}_${model}_${selection}${uncertainty}_balance${balance}_hard${use_hard_examples}_${fraction}_${seed}.out
                done
        done
done
...
do
        ...
        do
                for noisy_dataset_path in "noisycifar10_10.pt" "noisycifar10_30.pt" "noisycifar10_50.pt"
                do
                        python src/DeepCore/main.py --data_path $data_path --dataset $dataset --n-class $n_class --model $model --selection $selection --submodular $submodular --epochs $epochs --batch-size $batch_size --balance $balance --fraction $fraction --seed $seed --per_epoch $per_epoch --resolution $resolution --test_interval $test_interval --save_path $chpt_path  --noisy_dataset_path $noisy_dataset_path >& $out_path/timeToAcc_${per_epoch}_${resolution}_${test_interval}_${dataset}_${model}_${selection}${submodular}_balance${balance}_hard${use_hard_examples}_${fraction}_${seed}.out
                done
        done
done
...
do
  ...
  do
          ...
          do
                  for noisy_dataset_path in "noisycifar10_10.pt" "noisycifar10_30.pt" "noisycifar10_50.pt"
                  do
                        python src/DeepCore/main.py --data_path $data_path --dataset $dataset --n-class $n_class --model $model --selection $selection --epochs $epochs --batch-size $batch_size --balance $balance --fraction $fraction --seed $seed --per_epoch $per_epoch --use_hard_examples $use_hard_examples --test_interval $test_interval --save_path $chpt_path  --noisy_dataset_path $noisy_dataset_path >& $out_path/timeToAcc_${per_epoch}_${resolution}_${test_interval}_${dataset}_${model}_${selection}_balance${balance}_hard${use_hard_examples}_${fraction}_${seed}.out
                  done
          done
  done
done
...
do
  ...
  do
    python src/AL/main.py --data_path $data_path --dataset $dataset --n-class $n_class --model $model --method $method --uncertainty $uncertainty --epochs $epochs --batch-size $batch_size --n-query $n_query --seed $seed --resolution $resolution --cycle $cycle  --noisy_dataset_path $noisy_dataset_path >& $out_path/timeToAcc_${dataset}_${model}_${resolution}_${method}${uncertainty}_${seed}.out
  done
done
...
do
        ...
        do
                for noisy_dataset_path in "noisycifar10_10.pt" "noisycifar10_30.pt" "noisycifar10_50.pt"
                do
                        python src/DeepCore/main.py --data_path $data_path --dataset $dataset --n-class $n_class --model $model --selection $selection --epochs $epochs --batch-size $batch_size --balance $balance --fraction $fraction --seed $seed --per_epoch $per_epoch --resolution $resolution --test_interval $test_interval  --noisy_dataset_path $noisy_dataset_path >& $out_path/rs2_perepoch${per_epoch}_${resolution}_${test_interval}_${dataset}_${model}_${selection}_balance${balance}_hard${use_hard_examples}_${fraction}_${seed}.out
                done
