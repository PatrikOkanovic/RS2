...
from src.DeepCore.data_distillation.networks import MLP, ConvNet, LeNet, AlexNet, AlexNetBN, VGG11, VGG11BN, ResNet18, ResNet18BN_AP, \
    ResNet18BN, ResNet50
...
def get_network(model, channel, num_classes, im_size=(32, 32), seed=True):
    if seed:
...
def epoch(mode, dataloader, net, optimizer, criterion, args, aug):
    ...
    for i_batch, datum in enumerate(dataloader):  # enumerate(tqdm(dataloader, leave=False)):
...
def evaluate_synset(it_eval, net, images_train, labels_train, testloader, args):
    ...
    for ep in tqdm(range(Epoch + 1)):
        loss_train, acc_train = epoch('train', trainloader, net, optimizer, criterion, args, aug=False)  # aug=True)
...
def get_eval_pool(eval_mode, model, model_eval):
    if eval_mode == 'M':  # multiple architectures
        ...
        if 'BN' in model:
            print(
                'Attention: Here I will replace BN with IN in evaluation, '
                'as the synthetic set is too small to measure BN hyper-parameters.')
